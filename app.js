...
function mulaiAplikasi() {
    ...
    createApp({
        ...
        methods: {
            ...
            simpanTemaLokal(tema) {
                try { localStorage.setItem('kaslm_tema', tema); } catch (e) { }
            },
            toggleDark() {
                this.dark = !this.dark;
                document.documentElement.classList.toggle('dark', this.dark);
                ...
                if (this.token && this.profil && !this.profil.ViaPin) {
                    ...
                    this._simpanTemaTimer = setTimeout(() => {
                        this.jalankan('updateProfilSaya', [this.token, { Tema: tema }]).catch(() => { });
                    }, 500);
                }
            },
            terapkanTema(tema) {
                if (tema === 'terang') this.dark = false;
                    ...
                    const lokal = localStorage.getItem('kaslm_tema');
                    if (lokal === 'gelap') this.dark = true;
                    else if (lokal === 'terang') this.dark = false;
                    else this.dark = !!(window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches);
                }
                document.documentElement.classList.toggle('dark', this.dark);
                this.simpanTemaLokal(tema || (this.dark ? 'gelap' : 'terang'));
                if (this.formProfil) this.formProfil.Tema = tema || (this.dark ? 'gelap' : 'terang');
            },
            ...
            terapkanProfil(p) {
                ...
                const lokalTema = localStorage.getItem('kaslm_tema');
                const temaAktif = (p.Tema === 'gelap' || p.Tema === 'terang') ? p.Tema : (lokalTema || p.Tema || 'sistem');
                this.formProfil = { Nama: p.Nama, No_HP: p.No_HP, Tema: temaAktif, Notif_Email: p.Notif_Email !== false, Notif_WA: p.Notif_WA !== false };
            ...
            pilihLampiranChat(e) {
                ...
                if (file.size > 8 * 1024 * 1024) {
                    this.toast('Lampiran maksimal 8 MB.', 'error');
                    e.target.value = '';
                    return;
                }
            ...
            hapusLampiranChat() {
                if (this.chatLampiran && this.chatLampiran.previewUrl) {
                    URL.revokeObjectURL(this.chatLampiran.previewUrl);
                }
                this.chatLampiran = null;
            },
            unggahLampiranChat(e) {
                this.pilihLampiranChat(e);
            },
            async kirimChat() {
                ...
                if (lampiran) {
                    ...
                    try {
                        ...
                        this.loading = false;
                        this.loadingText = 'Memproses';
                        this.toast((err && err.message) || 'Gagal mengunggah lampiran.', 'error');
                        return;
                    } finally {
                        this.loading = false;
                        this.loadingText = 'Memproses';
                ...
                try {
                    ...
                    this.toast((e && e.message) || 'Gagal mengirim pesan.', 'error');
                    return;
                }
